...
unsigned long walk(struct task_struct *task, unsigned long vm_address)
{
    ...
    unsigned long vpn2 = (vm_address >> VPN2OFFSET) & MUSK;
    if ((pgtbl[vpn2] & VALID) == 0)
    {
        return NULL;
    }
    unsigned long *pglevel1 = (unsigned long *)(((pgtbl[vpn2] >> 10) << 12) + PA2VA_OFFSET);
    unsigned long vpn1 = (vm_address >> VPN1OFFSET) & MUSK;
    if ((pglevel1[vpn1] & VALID) == 0)
    {
        return NULL;
    }
    unsigned long *pglevel0 = (unsigned long *)(((pglevel1[vpn1] >> 10) << 12) + PA2VA_OFFSET);
    unsigned long vpn0 = (vm_address >> VPN0OFFSET) & MUSK;
    if ((pglevel0[vpn0] & VALID) == 0)
    {
        return NULL;
    }
    return pglevel0[vpn0];
}